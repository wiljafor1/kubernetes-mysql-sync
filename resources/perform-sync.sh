...
if [ "$has_failed" = false ]; then
    for CURRENT_DATABASE in ${SOURCE_DATABASE_NAMES//,/ }; do
        ...
        echo -e "mysqldump -u $SOURCE_DATABASE_USER -h $SOURCE_DATABASE_HOST -p$SOURCE_DATABASE_PASSWORD -P $SOURCE_DATABASE_PORT $BACKUP_ADDITIONAL_PARAMS $BACKUP_CREATE_DATABASE_STATEMENT --databases $CURRENT_DATABASE >/tmp/$DUMP"
